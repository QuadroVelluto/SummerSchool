...
void loadContactsFromFile(struct node **head, const char *filename)
{
    int fd = open(filename, O_RDONLY);
    if (fd == -1)
        return; // файл не существует — список пуст

    Person temp;
    while (read(fd, &temp, sizeof(Person)) == sizeof(Person))
        addContactSorted(head, &temp);

    close(fd);
}

void saveContactsToFile(struct node *head, const char *filename)
{
    ...
    while (head)
    {
        write(fd, &head->person, sizeof(Person));
        head = head->next;
    }
...
void menu()
{
    ...
    loadContactsFromFile(&head, FILENAME);
    struct node *idCounter = head;
    while (idCounter)
    {
        if (idCounter->person.id >= id_helper)
            id_helper = idCounter->person.id + 1;
        idCounter = idCounter->next;
    }
    ...
    while (true)
    {
        ...
        case 0:
        saveContactsToFile(head, FILENAME);
            while (head)
            {
